...
public class BusRouteInfoServlet2 extends HttpServlet {
	...
	ArrayList<ListItem2> ListItemList2(String routeId) {
		...
		try {
			...
			String[] tags = StaticValue.ROUTEINFO_SERVLET_STATION_LIST_TAG;
			ArrayList<HashMap<String, String>> answer=dbm.getDBDataList(StaticValue.STATION_OF_ROUTE_TABLE_NALE, tags, "stationSeq", true, 
					new String[] {"routeId"}, new String[] {routeId});
			answer.sort((x, y) -> new Integer (Integer.parseInt(x.get("stationSeq"))).compareTo(Integer.parseInt(y.get("stationSeq"))));
			
			for (HashMap<String, String> row : answer) {
				BusLocation bLoca = null;

				if (i < bl.size() && bl.get(i).getStationSeq().equals(row.get("stationSeq"))) {
					bLoca = bl.get(i++);
				}
				result.add(new ListItem2(row, bLoca,bl.size()));
			}
	...
	*/
	...
	class ListItem2 {
		...
		ListItem2(HashMap<String, String> busRouteStation, BusLocation busLocation, int busLocacnt) {
			this.busRouteStation = busRouteStation;
			this.busLocation = busLocation;
			this.busLocacnt = busLocacnt;
		}

		public HashMap<String, String> getBusRouteStation() {
			return busRouteStation;
		}
